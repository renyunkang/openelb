...
import (
	...
	apitopology "k8s.io/apiserver/pkg/endpoints/topology"
...
type Config struct {
	...
	TopologyInfoResolver apitopology.TopologyInfoResolver

	TopologyInfoPopulator apitopology.Populator
...
func DefaultBuildHandlerChain(apiHandler http.Handler, c *Config) http.Handler {
	...
	if c.TopologyInfoResolver != nil {
		if c.TopologyInfoPopulator != nil {
			handler = filterlatency.TrackCompleted(handler)
			handler = genericapifilters.WithPopulatedTopologyInfo(handler, c.TopologyInfoPopulator)
			handler = filterlatency.TrackStarted(handler, "topologyinfopopulator")
		}
		handler = filterlatency.TrackCompleted(handler)
		handler = genericapifilters.WithTopologyInfo(handler, c.TopologyInfoResolver)
		handler = filterlatency.TrackStarted(handler, "topologyinfo")
	}
