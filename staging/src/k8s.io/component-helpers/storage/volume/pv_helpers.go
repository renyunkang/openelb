...
import (
	"fmt"
	clusterv1alpha1 "k8s.io/api/mizarcore/v1alpha1"
...
const (
	...
	// AnnSelectedCluster annotation is added to a PVC that has been triggered by scheduler to
	// be dynamically provisioned. Its value is the name of the selected cluster.
	AnnSelectedCluster = "volume.kubernetes.io/selected-cluster"
...
// as a match for the claim, but unbound PVs are skipped.
//
// cluster is set only in the scheduler path. When set, the PV cluster affinity is checked against
// the cluster's labels.
//
// excludedVolumes is only used in the scheduler path, and is needed for evaluating multiple
// unbound PVCs for a single Pod at one time.  As each PVC finds a matching PV, the chosen
// PV needs to be excluded from future matching.
func FindMatchingClusterVolume(
	claim *v1.PersistentVolumeClaim,
	volumes []*v1.PersistentVolume,
	...
	for _, volume := range volumes {
		...
		clusterAffinityValid := true
		if cluster != nil {
			// Scheduler path, check that the PV ClusterAffinity
			// is satisfied by the cluster
			// volumeutil.CheckClusterAffinity is the most expensive call in this loop.
			// We should check cheaper conditions first or consider optimizing this function.
			err := CheckClusterAffinity(volume, cluster.Labels)
			if err != nil {
				clusterAffinityValid = false
			}
		}
