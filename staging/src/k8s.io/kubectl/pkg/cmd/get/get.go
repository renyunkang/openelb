...
func NewCmdGet(parent string, f cmdutil.Factory, streams genericclioptions.IOStreams, config *genericclioptions.ConfigFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			cmdutil.CheckErr(o.Run(f, cmd, args, config))
		},
	...
	cmd.Flags().StringSliceVarP(&o.PlanetClusters, "planet-clusters", "C", []string{}, "-C=member1,member2")
...
func (o *GetOptions) Run(f cmdutil.Factory, cmd *cobra.Command, args []string, config *genericclioptions.ConfigFlags) error {
	if len(o.Raw) > 0 {
	...
	if len(o.PlanetClusters) > 0 {

		restConfig, _ := f.ToRESTConfig()
		return o.getPlanetInfos(args, restConfig, config)
	}
	r := f.NewBuilder().
		Unstructured().
...
func (o *GetOptions) getPlanetInfos(args []string, restConfig *rest.Config, config *genericclioptions.ConfigFlags) error {
	mux := sync.Mutex{}
	var wg sync.WaitGroup
	var allErrs []error
	var objs []Obj
	wg.Add(len(o.PlanetClusters))
	for idx := range o.PlanetClusters {
		startClusterFactory := o.getFactory(o.PlanetClusters[idx], "", restConfig, config)
		go o.getObjInfo(&wg, &mux, startClusterFactory, o.PlanetClusters[idx], &objs, &allErrs, args)
	}
	wg.Wait()
	...
	o.printObjs(objs, &allErrs, args)

	return utilerrors.NewAggregate(allErrs)
}

func (o *GetOptions) getFactory(clusterName string, namespace string, restConfig *rest.Config, config *genericclioptions.ConfigFlags) cmdutil.Factory {
	kubeConfigFlags := genericclioptions.NewConfigFlags(true).WithDeprecatedPasswordFlag()

	// Build member cluster kubeConfigFlags
	planetApiServer := restConfig.Host + fmt.Sprintf(proxyURL, clusterName)
	kubeConfigFlags.APIServer = &planetApiServer
	// Use kubeconfig to access member cluster
	kubeConfigFlags.KubeConfig = config.KubeConfig
	kubeConfigFlags.Context = config.Context
...
func (o *GetOptions) getObjInfo(wg *sync.WaitGroup, mux *sync.Mutex, f cmdutil.Factory, cluster string, objs *[]Obj, allErrs *[]error, args []string) {
	defer wg.Done()
	restClient, err := f.RESTClient()
	...
	if !o.IsHumanReadablePrinter {
		if err := o.printGeneric(r); err != nil {
			*allErrs = append(*allErrs, fmt.Errorf("cluster(%s): %s", cluster, err))
		}
		return
	}
...
func (o *GetOptions) printObjs(objs []Obj, allErrs *[]error, args []string) {
	var err error
	errs := sets.NewString()

	var printWithKind = false
	...
	for ix := range objs {
		...
		printWithNamespace := o.checkPrintWithNamespace(mapping)
		...
		if ix == len(objs)-1 || objs[ix].Info.Mapping.Resource != objs[ix+1].Info.Mapping.Resource {
			table := &metav1.Table{}
			allTableRows, mapping, err := o.reconstructionRow(sameKind, table)
			if err != nil {
				*allErrs = append(*allErrs, err)
				return
			}
			...
			o.setColumnDefinition(table)

			printObj, err := ToUnstructured(table)
			if err != nil {
				*allErrs = append(*allErrs, err)
				return
			}
	...
	o.printIfNotFindResource(trackingWriter.Written, allErrs, allResourcesNamespaced)
}

// printIfNotFindResource is sure we output something if we wrote no output, and had no errors, and are not ignoring NotFound
func (g *GetOptions) printIfNotFindResource(written int, allErrs *[]error, allResourcesNamespaced bool) {
...
func (g *GetOptions) checkPrintWithNamespace(mapping *meta.RESTMapping) bool {
	if mapping != nil && mapping.Scope.Name() == meta.RESTScopeNameRoot {
		return false
	}
...
const (
	...
	proxyURL              = "/apis/core.mizargalaxy.mizar-k8s.io/v1alpha1/clusters/%s/proxy/"
)
...
func (o *GetOptions) reconstructionRow(objs []Obj, table *metav1.Table) ([]metav1.TableRow, *meta.RESTMapping, error) {
...
func (o *GetOptions) setColumnDefinition(table *metav1.Table) {
