...
var map_CSIStorageCapacity = map[string]string{
	...
	"clusterTopology":   "ClusterTopology defines which clusters have access to the storage for which capacity was reported. If not set, the storage is not accessible from any cluster in the cluster. If empty, the storage is accessible from all clusters.  This field is immutable.",
}
...
var map_StorageClass = map[string]string{
	...
	"provisionScope":       "ProvisionScope indicates where volumes should be used. Remote means volumes should be used in planet cluster. Local means volumes should be used in star cluster. Default is Remote",
}
