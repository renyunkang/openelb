...
type PersistentVolumeSpec struct {
	...
	// ClusterAffinity defines constraints that limit what clusters this volume can be accessed from.
	// This field influences the scheduling of pods that use this volume.
	// +optional
	ClusterAffinity *VolumeClusterAffinity `json:"clusterAffinity,omitempty" protobuf:"bytes,10,opt,name=clusterAffinity"`
	// Volume will determine where the pv should be used
	// Default is Remote
	// +optional
	VolumeLocation *VolumeLocation `json:"volumeLocation,omitempty" protobuf:"bytes,11,opt,name=volumeLocation,casttype=VolumeLocation"`
}

type VolumeLocation string

const (
	// LocalVolume means the pv used by star cluster.
	LocalVolume VolumeLocation = "Local"
	// RemoteVolume means the pv used by planet cluster.
	RemoteVolume VolumeLocation = "Remote"
)
...
// VolumeClusterAffinity defines constraints that limit what clusters this volume can be accessed from.
type VolumeClusterAffinity struct {
	// Required specifies hard cluster constraints that must be met.
	Required *ClusterSelector `protobuf:"bytes,1,opt,name=required"`
}
...
type PersistentVolumeClaimSpec struct {
	...
	// VolumeScope determine pv created by either star cluster or planet cluster
	// Default is Star
	// +optional
	VolumeScope *VolumeScope `json:"volumeScope,omitempty" protobuf:"bytes,8,opt,name=volumeScope,casttype=VolumeScope"`
	...
	// +optional
	DataSourceRef *TypedLocalObjectReference `json:"dataSourceRef,omitempty" protobuf:"bytes,9,opt,name=dataSourceRef"`
}

type VolumeScope string

const (
	// VolumeOnStar means pv is created by star cluster and synced to planet cluster, pvc is synced to planet cluster without storageclass.(volume scope cross multiple clusters)
	VolumeOnStar VolumeScope = "Star"
	// VolumeOnPlanet means pvc is synced to planet cluster with storageclass and pv is created by planet cluster.(volume scope limit in a cluster)
	VolumeOnPlanet VolumeScope = "Planet"
)
...
type PersistentVolumeClaimStatus struct {
	...
	// ClusterNames represents the current clusters of PersistentVolumeClaim
	// +optional
	ClusterNames []string `json:"clusterNames,omitempty" protobuf:"bytes,7,rep,name=clusterNames"`
}
...
type Affinity struct {
	...
	// +optional
	ClusterAffinity *ClusterAffinity `json:"clusterAffinity,omitempty" protobuf:"bytes,4,opt,name=clusterAffinity"`
}
...
type ClusterAffinity struct {
	...
	// +optional
	PreferredDuringSchedulingIgnoredDuringExecution []PreferredSchedulingClusterTerm `json:"preferredDuringSchedulingIgnoredDuringExecution,omitempty" protobuf:"bytes,2,rep,name=preferredDuringSchedulingIgnoredDuringExecution"`
}
...
type PreferredSchedulingClusterTerm struct {
	...
	Preference ClusterSelectorTerm `json:"preference" protobuf:"bytes,2,rep,name=preference"`
}
...
type PodSpec struct {
	...
	// +optional
	ClusterSelector map[string]string `json:"clusterSelector,omitempty" protobuf:"bytes,37,rep,name=clusterSelector"`
	// +optional
	ClusterTolerations []ClusterToleration `json:"clusterTolerations,omitempty" protobuf:"bytes,38,rep,name=clusterTolerations"`
	// +optional
	Location *Location `json:"location,omitempty" protobuf:"bytes,39,opt,name=location"`
}
...
type PodStatus struct {
	...
	NominateLocation *Location `json:"nominateLocation,omitempty" protobuf:"bytes,14,opt,name=nominateLocation"`
}
...
type ClusterResourceQuotaSpec struct {
	...
	// More info: https://kubernetes.io/docs/concepts/policy/resource-quotas/
	// +optional
	Hard ResourceList `protobuf:"bytes,1,rep,name=hard,casttype=ResourceList,castkey=ResourceName"`
	// A collection of filters that must match each object tracked by a quota.
	// If not specified, the quota matches all objects.
	// +optional
	Scopes []ResourceQuotaScope `protobuf:"bytes,2,rep,name=scopes,casttype=ResourceQuotaScope"`
	// scopeSelector is also a collection of filters like scopes that must match each object tracked by a quota
	// but expressed using ScopeSelectorOperator in combination with possible values.
	// For a resource to match, both scopes AND scopeSelector (if specified in spec), must be matched.
	// +optional
	ScopeSelector *ScopeSelector `protobuf:"bytes,3,opt,name=scopeSelector"`

	// ClusterSelector is a selector which must be true for the workload to fit on a cluster.
	// Selector which must match a cluster's labels for the workload to be scheduled on that cluster.
	// +optional
	ClusterSelector map[string]string `protobuf:"bytes,4,rep,name=clusterSelector"`
}
