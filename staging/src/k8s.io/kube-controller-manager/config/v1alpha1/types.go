...
type KubeControllerManagerConfiguration struct {
	...
	// ClusterLifecycleControllerConfiguration holds configuration for
	// ClusterLifecycleController related features.
	ClusterLifecycleController ClusterLifecycleControllerConfiguration
}
...
// ClusterLifecycleControllerConfiguration contains elements describing ClusterLifecycleController.
type ClusterLifecycleControllerConfiguration struct {
	// If set to true enables NoExecute Taints and will evict all not-tolerating
	// Pod running on Clusters tainted with this kind of Taints.
	EnableTaintManager *bool
	// clusterEvictionRate is the number of clusters per second on which pods are deleted in case of cluster failure when a zone is healthy
	ClusterEvictionRate float32
	// secondaryClusterEvictionRate is the number of clusters per second on which pods are deleted in case of cluster failure when a zone is unhealthy
	SecondaryClusterEvictionRate float32
	// clusterStartupGracePeriod is the amount of time which we allow starting a cluster to
	// be unresponsive before marking it unhealthy.
	ClusterStartupGracePeriod metav1.Duration
	// ClusterMonitorGracePeriod is the amount of time which we allow a running cluster to be
	// unresponsive before marking it unhealthy. Must be N times more than kubelet's
	// clusterStatusUpdateFrequency, where N means number of retries allowed for kubelet
	// to post cluster status.
	ClusterMonitorGracePeriod metav1.Duration
	// podEvictionTimeout is the grace period for deleting pods on failed clusters.
	PodEvictionTimeout metav1.Duration
	// secondaryClusterEvictionRate is implicitly overridden to 0 for clusters smaller than or equal to largeClusterSizeThreshold
	LargeClusterSizeThreshold int32
	// Zone is treated as unhealthy in clusterEvictionRate and secondaryClusterEvictionRate when at least
	// unhealthyZoneThreshold (no less than 3) of Clusters in the zone are NotReady
	UnhealthyZoneThreshold float32
	// Control whether to evict the Pods of StableModels
	EnableStableModelPodEvict bool
	// Control whether to evict StableModels
	EnableStableModelEvict bool
	// StableModelEvictionTimeout is the grace period for deleting stablemodels on failed clusters.
	StableModelEvictionTimeout metav1.Duration
	// Control whether to evict the Pods of Workloads
	EnableWorkloadPodEvict bool
	// Control whether to evict Workloads
	EnableWorkloadEvict bool
	// WorkloadEvictionTimeout is the grace period for deleting Workloads on failed clusters.
	WorkloadEvictionTimeout metav1.Duration
	// Control whether to evict Cluster Pod
	EnableClusterPodEvict bool
}
