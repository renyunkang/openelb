...
type WebhookClientConfigApplyConfiguration struct {
	...
	CertPEMBlock []byte                              `json:"certPEMBlock,omitempty"`
	KeyPEMBlock  []byte                              `json:"keyPEMBlock,omitempty"`
}
...
// WithCertPEMBlock adds the given value to the CertPEMBlock field in the declarative configuration
// and returns the receiver, so that objects can be build by chaining "With" function invocations.
// If called multiple times, values provided by each call will be appended to the CertPEMBlock field.
func (b *WebhookClientConfigApplyConfiguration) WithCertPEMBlock(values ...byte) *WebhookClientConfigApplyConfiguration {
	for i := range values {
		b.CertPEMBlock = append(b.CertPEMBlock, values[i])
	}
	return b
}

// WithKeyPEMBlock adds the given value to the KeyPEMBlock field in the declarative configuration
// and returns the receiver, so that objects can be build by chaining "With" function invocations.
// If called multiple times, values provided by each call will be appended to the KeyPEMBlock field.
func (b *WebhookClientConfigApplyConfiguration) WithKeyPEMBlock(values ...byte) *WebhookClientConfigApplyConfiguration {
	for i := range values {
		b.KeyPEMBlock = append(b.KeyPEMBlock, values[i])
	}
	return b
}
