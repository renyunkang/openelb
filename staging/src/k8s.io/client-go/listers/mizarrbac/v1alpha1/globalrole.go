...
type GlobalRoleLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.GlobalRole, err error)
	// Get retrieves the GlobalRole from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.GlobalRole, error)
...
// List lists all GlobalRoles in the indexer.
func (s *globalRoleLister) List(selector labels.Selector) (ret []*v1alpha1.GlobalRole, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.GlobalRole))
	})
	return ret, err
}

// Get retrieves the GlobalRole from the index for a given name.
func (s *globalRoleLister) Get(name string) (*v1alpha1.GlobalRole, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("globalrole"), name)
	}
	return obj.(*v1alpha1.GlobalRole), nil
}
